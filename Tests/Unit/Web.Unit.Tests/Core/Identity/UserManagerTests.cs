...
namespace SecurityEssentials.Unit.Tests.Core.Identity
{
    ...
    public class UserManagerTests
    {
        ...
        public void Given_UserIsReusingAPassword_When_ValidatePassword_Then_ErrorReturned(HashStrategyKind hashStrategy)
        {
            ...
            var user = new User
            {
                PreviousPasswords = new List<PreviousPassword>
                {
                    ...
                    {
                        Hash = Convert.ToBase64String(previousPassword.Hash),
                        Salt = Convert.ToBase64String(previousPassword.Salt),
                        HashStrategy = previousPassword.HashStrategy
                    }
                }
            };
        ...
        public async Task Given_ValidDetails_When_ChangePasswordFromToken_Then_PasswordChanged()
        {
            ...
            _userStore.Expect(a => a.ChangePasswordFromTokenAsync(userId, token, newPassword))
        ...
        public async Task Given_ValidDetails_When_ResetPasswordAsync_Then_PasswordReset()
        {
            ...
            _userStore.Expect(a => a.ResetPasswordAsync(Arg<int>.Is.Equal(userId), Arg<string>.Is.Anything,
                Arg<string>.Is.Equal(actioningUserName))).Return(Task.FromResult(IdentityResult.Success));
            _services.Expect(a => a.SendEmail(Arg<string>.Is.Anything, Arg<List<string>>.Is.Anything,
                Arg<List<string>>.Is.Anything, Arg<List<string>>.Is.Anything, Arg<string>.Is.Anything,
                Arg<string>.Is.Anything, Arg<bool>.Is.Anything)).Return(true);
        ...
        public async Task When_LogonAsync_Then_LogsUpdated()
        {
            ...
            _userStore.Expect(a => a.CreateIdentityAsync(Arg<User>.Matches(b => b.Id == testUser.Id),
                    Arg<string>.Is.Equal(DefaultAuthenticationTypes.ApplicationCookie)))
            ...
            authenticationManager.Expect(a =>
                a.SignIn(Arg<AuthenticationProperties>.Is.Anything, Arg<ClaimsIdentity>.Is.Anything));
