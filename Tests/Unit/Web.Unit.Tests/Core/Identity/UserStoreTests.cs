...
namespace SecurityEssentials.Unit.Tests.Core.Identity
{
    ...
    public class UserStoreTests
    {
        ...
        [TestCase(null, false)]
        public async Task Given_UserExists_When_FindAndCheckLogonAsync_Then_UserIsLoggedOn(DateTime? passwordExpiryDate,
            bool expectedMustChangePassword)
        {
            // Arrange
        ...
        public async Task Given_ValidData_When_ResetPassword_Then_Success()
        {
            ...
            Assert.AreEqual(0, TestUser.FailedLogonAttemptCount);
            Assert.AreEqual(1, TestUser.UserLogs.Count);
            Assert.IsTrue(TestUser.UserLogs.Any(a =>
                a.Description.Contains("User had password reset sent out via email by bob")));
            ...
            Assert.IsTrue(TestUser.PasswordLastChangedDateUtc > DateTime.UtcNow.AddMinutes(-5));
            Assert.That(TestUser.PasswordExpiryDateUtc, Is.LessThanOrEqualTo(DateTime.UtcNow),
                "User should be required to change password on next logon");
        }
