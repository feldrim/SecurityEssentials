    ...
    public class EncryptionTests
    {
        ...
        public void EnDecrypt_Message_ChangePassword_Fails()
        {
            ...
            Assert.AreNotEqual(true,
                _testDecrypt.Decrypt(string.Format("{0}1", _password), salt, _iterationCount, encrypted, out output));
            Assert.AreNotEqual(input, output);
        }
        ...
        public void EnDecrypt_Message_ChangeSalt_Fails()
        {
            ...
            Assert.AreNotEqual(input, encrypted);
            Assert.AreNotEqual(true,
                _testDecrypt.Decrypt(_password, $"{salt}1", _iterationCount, encrypted, out output));
            Assert.AreNotEqual(input, output);
        }
    }
}