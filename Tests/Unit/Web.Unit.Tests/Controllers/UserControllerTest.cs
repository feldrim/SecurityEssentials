...
namespace SecurityEssentials.Unit.Tests.Controllers
{
    ...
    public class UserControllerTest : BaseControllerTest
    {
        ...
        public void Setup()
        {
            ...
            _sut = new UserController(_appSensor, _configuration, Context, _httpCache, UserIdentity, _userManager,
                _services)
            {
        ...
        public void Given_AdminUserAndDataExists_When_Read_Then_JsonReturned(string searchName, int pageNumber,
            int numberOfRowsInPage, string sortIndex, string sortDirection, int expectedNumberOfRows,
            string expectedFirstFullName)
        {
            // Arrange
            ...
            if (expectedNumberOfRows > 0) Assert.That(model["data"][0]["FullName"], Is.EqualTo(expectedFirstFullName));
        }
        ...
        public void Given_SubmissionCorrect_When_EditPost_Then_ViewReturned(bool isAdmin)
        {
            ...
            {
                {"User.UserName", "new@new.test.net"},
                {"User.FirstName", "new"},
                {"User.LastName", "name"}
            };
            ...
            if (isAdmin)
            {
                _services.AssertWasCalled(a => a.SendEmail(Arg<string>.Is.Anything,
                    Arg<List<string>>.Matches(b => b.Contains("new@new.test.net")), Arg<List<string>>.Is.Anything,
        ...
        [Test]
        [Ignore("Wait for test case where user cannot be deleted")]
        public void Given_UserHasExistingData_When_DeleteUserPost_Then_ErrorMessageReturned()
        {
            ...
            Assert.That(viewModel.ViewBag.Message,
                Is.EqualTo("This user has data associated with it and cannot be deleted"));
        }
        ...
        {
            // Arrange
            _userManager.Expect(a => a.ResetPasswordAsync(TestUserId, TestUserName))
                .Return(Task.FromResult(new SeIdentityResult("Error")));
            ...
            Assert.That(viewResult.ViewBag.Message,
                Is.EqualTo("An error occurred whilst trying to perform this action"));
        }
