...
namespace SecurityEssentials.Unit.Tests.Controllers
{
    ...
    public class AccountControllerTest : BaseControllerTest
    {
        ...
        public void Setup()
        {
            ...
            _sut = new AccountController(_appSensor, _configuration, _encryption, _formsAuth, Context, _httpCache,
        ...
        public async Task Given_AccountDetailsIncorrect_When_ChangeSecurityInformation_Then_ViewWithErrorReturned()
        {
            ...
            Assert.That(resultModel.ErrorMessage,
                Contains.Substring("Security information incorrect or account locked out"));
        }
        ...
        public async Task Given_CredentialsCorrect_When_LogonPost_Then_RedirectsToLandingPage(bool mustChangePassword)
        {
            ...
            if (mustChangePassword)
                _httpCache.AssertWasCalled(a =>
                    a.SetCache(Arg<string>.Is.Equal("MustChangePassword-1"), Arg<string>.Is.Equal("")));
        }
        ...
        [Test]
        public async Task
            Given_CredentialsCorrectAndExternalReturnUrlProvided_When_LogonPost_Then_RedirectsToLandingPage()
        {
            // Arrange
            var logon = new LogOnViewModel {UserName = "joeblogs", Password = "password", RememberMe = false};
        ...
        public async Task Given_InvalidUser_When_RecoverPasswordPost_Then_ReturnsErrorView()
        {
            ...
            Assert.That(model.Exception.Message,
                Contains.Substring("The user is either not valid, not approved or not active"));
        }
        ...
        public async Task Given_RequestVerificationToken_When_EmailVerify_Then_UserStatusUpdated()
        {
            ...
            _services.AssertWasNotCalled(a => a.SendEmail(Arg<string>.Is.Anything, Arg<List<string>>.Is.Anything,
                Arg<List<string>>.Is.Anything, Arg<List<string>>.Is.Anything, Arg<string>.Is.Anything,
        ...
        public async Task Given_ResetPasswordFromTokenFails_When_RecoverPasswordPost_Then_ReturnsError()
        {
            ...
            _userManager
                ...
                .Return(Task.FromResult(new SeIdentityResult(new List<string> {"some stuff went wrong"})));
            _recaptcha.Expect(a => a.ValidateRecaptcha(Arg<Controller>.Is.Anything)).Return(true);
        ...
        public void Given_UserSubmissionInvalid_When_RecoverPasswordGet_Then_ErrorViewShown(string tokenPassed,
            string tokenStored, int expiryMinutes)
        {
            // Arrange
            var requestItems = new NameValueCollection();
            ...
            var model = AssertViewResultReturnsType<HandleErrorInfo>(result);
            Assert.That(model.Exception.Message,
                Contains.Substring("password recovery token is not valid or has expired"));
        }
        ...
        public async Task Given_ValidSubmission_When_ChangeSecurityInformation_Then_ViewReturned()
        {
            ...
            _userManager.Expect(a => a.TryLogOnAsync(Arg<string>.Is.Anything, Arg<string>.Is.Anything))
                .Return(Task.FromResult(new LogonResult {Success = true, UserName = TestUserName}));
            _encryption.Expect(e => e.Encrypt(Arg<string>.Is.Anything, Arg<int>.Is.Anything, Arg<string>.Is.Anything,
                out Arg<string>.Out(EncryptedSecurityAnswerSalt).Dummy,
                out Arg<string>.Out(EncryptedSecurityAnswer).Dummy)).Return(false);
            _recaptcha.Expect(a => a.ValidateRecaptcha(Arg<Controller>.Is.Anything)).Return(true);
