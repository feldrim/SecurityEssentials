...
namespace SecurityEssentials.Unit.Tests.Controllers
{
    public abstract class BaseControllerTest : IDisposable
    {
        ...
        public void AssertViewResultWithError(ActionResult actionResult, string errorValue)
        {
            ...
            var error = viewResult.ViewData.ModelState.ToList().Select(a => a.Value.Errors).Where(b => b.Count > 0)
                .ToList();
        ...
        public ViewResult AssertViewResultReturned(ActionResult actionResult, string viewName)
        {
            ...
            Assert.IsInstanceOf<ViewResult>(actionResult, "Not ViewResult returned from controller");
            var viewResult = (ViewResult) actionResult;
            if (!string.IsNullOrEmpty(viewName)) Assert.AreEqual(viewName, viewResult.ViewName);
            return viewResult;
        }
        ...
        public void AssertPartialViewResultReturned(ActionResult actionResult, string partialViewName)
        {
            ...
            if (!string.IsNullOrEmpty(partialViewName)) Assert.AreEqual(partialViewName, viewResult.ViewName);
        }
        ...
        public void AssertRedirectToActionReturned(ActionResult result, object action, string controller)
        {
            ...
            Assert.IsInstanceOf<RedirectToRouteResult>(result, "Not RedirectToRouteResult returned from controller");
        ...
        public void AssertRedirectToActionReturned(ActionResult result, object routeValue, object action,
            string controller)
        {
            Assert.IsNotNull(result, "No result was returned from controller");
            Assert.IsInstanceOf<RedirectToRouteResult>(result, "Not RedirectToRouteResult returned from controller");
            var redirectResult = (RedirectToRouteResult) result;
            Assert.AreEqual(routeValue, redirectResult.RouteValues.Values.ToList()[0],
                "Route variable was not correct");
            Assert.AreEqual(action, redirectResult.RouteValues.Values.ToList()[1], "Route action was not correct");
            Assert.AreEqual(controller, redirectResult.RouteValues.Values.ToList()[2],
                "Route controller name was not correct");
        }
        ...
        {
            UserIdentity
                .Expect(a => a.GetRequester(Arg<Controller>.Is.Anything, Arg<AppSensorDetectionPointKind?>.Is.Anything))
        ...
        {
            UserIdentity.Expect(a => a.IsUserInRole(Arg<Controller>.Is.Anything, Arg<string>.Is.Equal("Admin")))
                .Return(isAdmin);
        }
