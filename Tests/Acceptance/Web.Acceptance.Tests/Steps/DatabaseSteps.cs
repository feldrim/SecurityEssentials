    ...
    public class DatabaseSteps
    {
        ...
        public void GivenTheFollowingUsersAreSetupInTheDatabase(Table table)
        {
            ...
            foreach (var userToCreate in usersToCreate)
            {
                ...
                var securityQuestion = SeDatabase.GetLookupItemsByLookupType(Consts.LookupTypeId.SecurityQuestion)
                    .Single(a => a.Description == userToCreate.SecurityQuestion);
                encryptor.Encrypt(ConfigurationManager.AppSettings["EncryptionPassword"],
                    Convert.ToInt32(ConfigurationManager.AppSettings["EncryptionIterationCount"]),
                    userToCreate.SecurityAnswer, out encryptedSecurityAnswerSalt, out encryptedSecurityAnswer);
                ...
                var user = new User
                {
                    ...
                    PasswordResetToken = string.IsNullOrWhiteSpace(userToCreate.PasswordResetToken)
                        ? null
                        : userToCreate.PasswordResetToken,
                    PasswordResetExpiryDateUtc = userToCreate.PasswordResetExpiry == "[One day from now]"
                        ? (DateTime?) DateTime.UtcNow.AddDays(1)
                        : null,
                    NewEmailAddress = string.IsNullOrWhiteSpace(userToCreate.NewEmailAddress)
                        ? null
                        : userToCreate.NewEmailAddress,
                    NewEmailAddressToken = string.IsNullOrWhiteSpace(userToCreate.NewEmailAddressToken)
                        ? null
                        : userToCreate.NewEmailAddressToken,
                    NewEmailAddressRequestExpiryDateUtc =
                        userToCreate.NewEmailAddressRequestExpiryDate == "[One day from now]"
                            ? (DateTime?) DateTime.UtcNow.AddDays(1)
                            : null,
                    PasswordExpiryDateUtc = userToCreate.PasswordExpiryDate == "[Expired]"
                    ...
                    if (tableRow.ContainsKey("SecurityQuestionLookupItemId"))
                        if (!string.IsNullOrWhiteSpace(tableRow["SecurityQuestionLookupItemId"]))
                        {
                            var securityQuestionLookupItem =
                                SeDatabase.GetLookupItemByLookupTypeAndDescription(Consts.LookupTypeId.SecurityQuestion,
                                    tableRow["SecurityQuestionLookupItemId"]);
        ...
        [Then(
            @"the user '(.*)' has the password reset token set and password reset expiry is at least (.*) minutes from now")]
        public void ThenTheUserHasThePasswordResetTokenSetAndPasswordResetExpirySetToMinutes(string userName,
            int expiryMinutes)
        {
            var user = SeDatabase.GetUsers().Single(a => a.UserName == userName);
            Assert.IsTrue(!string.IsNullOrEmpty(user.PasswordResetToken),
                $"User {user.UserName} should have had the password reset token set");
            Assert.IsTrue(user.PasswordResetExpiryDateUtc.HasValue,
                $"User {user.UserName} should have had the password reset token expiry date set");
            Assert.That(user.PasswordResetExpiryDateUtc.Value,
                Is.GreaterThan(DateTime.UtcNow.AddMinutes(expiryMinutes)));
        }
        ...
        {
            var user = SeDatabase.GetUsers().Single(a => a.UserName == userName);
            Assert.IsTrue(user.PasswordExpiryDateUtc.HasValue,
                $"User {user.UserName} should have had the password expiry date set");
        ...
        {
            var user = SeDatabase.GetUsers().Single(a => a.UserName == userName);
            Assert.IsFalse(user.PasswordExpiryDateUtc.HasValue,
                $"User {user.UserName} should not have the password expiry date set");
        }
        ...
        public void ThenThePasswordResetTokenAndExpiryForUserAreNotSet(string userName)
        {
            var user = SeDatabase.GetUsers().Single(a => a.UserName == userName);
            Assert.IsTrue(string.IsNullOrEmpty(user.PasswordResetToken),
                $"User {user.UserName} should have had the password reset token cleared");
            Assert.IsFalse(user.PasswordResetExpiryDateUtc.HasValue,
                $"User {user.UserName} should have had the password reset token expiry date cleared");
        }
        ...
        public void ThenTheUserHasTheNewEmailAddressTokenAndExpiryCleared(string userName)
        {
            ...
            Assert.IsNull(user.NewEmailAddressToken,
                $"User {user.UserName} should have had the new email address token cleared");
            Assert.IsFalse(user.NewEmailAddressRequestExpiryDateUtc.HasValue,
                $"User {user.UserName} should have had the new email address expiry date cleared");
        }

        [Then(
            @"the user '(.*)' has the new email address token set and new email address expiry is at least (.*) minutes from now")]
        public void ThenTheUserHasTheNewEmailAddressTokenSetAndNewEmailAddressExpiryIsAtLeastMinutesFromNow(
            string userName, int expiryMinutes)
        {
            var user = SeDatabase.GetUsers().Single(a => a.UserName == userName);
            Assert.IsTrue(!string.IsNullOrEmpty(user.NewEmailAddress),
                $"User {user.UserName} should have had the new email address set");
            Assert.IsTrue(!string.IsNullOrEmpty(user.NewEmailAddressToken),
                $"User {user.UserName} should have had the new email address token set");
            Assert.IsTrue(user.NewEmailAddressRequestExpiryDateUtc.HasValue,
                $"User {user.UserName} should have had the new email address expiry date set");
            Assert.That(user.NewEmailAddressRequestExpiryDateUtc.Value,
                Is.GreaterThan(DateTime.UtcNow.AddMinutes(expiryMinutes)));
        }
        ...
        public void ThenIHaveContentSecurityPolicyViolationInTheSystem(int expectedNumberOfCspViolations)
        {
            ...
            Assert.That(cspViolations.Count, Is.EqualTo(expectedNumberOfCspViolations),
                $"Was not able to find {expectedNumberOfCspViolations} csp violations in the logs");
        }
        ...
        public void ThenIHaveHttpPublicKeyPinningViolationInTheSystem(int expectedNumberOfHpkpViolations)
        {
            ...
            Assert.That(hpkpWarnings.Count, Is.EqualTo(expectedNumberOfHpkpViolations),
                $"Was not able to find {expectedNumberOfHpkpViolations} hpkp violations in the logs");
        }
