...
namespace SecurityEssentials.Controllers
{
    public class AccountController : SecurityControllerBase
    {
        ...
        public AccountController(IAppSensor appSensor, IAppConfiguration configuration, IEncryption encryption,
            IFormsAuth formsAuth, ISeContext context, IHttpCache httpCache, IUserManager userManager,
            IRecaptcha recaptcha, IServices services, IUserIdentity userIdentity) : base(userIdentity, appSensor)
        {
        ...
        {
            if (Request.IsAuthenticated) return RedirectToAction("Landing", "Account");
            ...
            if (ModelState.IsValid)
            {
                ...
                if (logonResult.Success)
                {
                    ...
                    Logger.Information("Successful Account Logon Post for username {userName}", userName,
                        model.UserName);
                    if (logonResult.MustChangePassword) _httpCache.SetCache($"MustChangePassword-{userId}", "");
        ...
        public ActionResult ChangeEmailAddress()
        {
            ...
            return View(new ChangeEmailAddressViewModel(user.UserName, user.NewEmailAddress,
                user.NewEmailAddressRequestExpiryDateUtc));
        }
        ...
        public async Task<ActionResult> ChangeEmailAddressAsync(ChangeEmailAddressViewModel model)
        {
            ...
            return View("ChangeEmailAddress",
                new ChangeEmailAddressViewModel(user.UserName, user.NewEmailAddress,
                    user.NewEmailAddressRequestExpiryDateUtc));
        }
        ...
        public async Task<ActionResult> ChangePasswordAsync(ChangePasswordViewModel model)
        {
            ...
            if (_configuration.HasRecaptcha) recaptchaSuccess = _recaptcha.ValidateRecaptcha(this);
        ...
        public ActionResult RecoverPassword()
        {
            ...
            var user = _context.User.Include("SecurityQuestionLookupItem").SingleOrDefault(u =>
                u.PasswordResetToken == passwordResetToken && u.PasswordResetExpiryDateUtc > DateTime.UtcNow);
            if (user == null)
            {
                var error = new HandleErrorInfo(
        ...
        public async Task<ActionResult> RecoverPasswordAsync(RecoverPasswordViewModel recoverPasswordModel)
        {
            ...
            if (recaptchaSuccess)
                if (ModelState.IsValid)
                {
                    var result = await _userManager.ChangePasswordFromTokenAsync(user.Id,
                ...
                if (recaptchaSuccess)
                    if (logonResult.Success)
                    {
                        if (model.SecurityAnswer == model.SecurityAnswerConfirm)
                        {
                            ...
                            _encryption.Encrypt(_configuration.EncryptionPassword,
        ...
        public ActionResult Register()
        {
            ...
            var registerViewModel =
                new RegisterViewModel("", _configuration.HasRecaptcha, "", new User(), securityQuestions);
                    ...
                    if (recaptchaSuccess)
                    {
                        ...
                        if (result.Succeeded || result.Errors.Any(e => e == "Username already registered"))
                        {
                            ...
                            if (result.Succeeded)
                            {
                                ...
                                Logger.Information(
                                    "Successful Account Register Post for username {userName} by requester {@requester}",
                                    userName, requester);
                            }
                            else
                            {
                                ...
                                emailBody = EmailTemplates.RegistrationDuplicatedBodyText(user.FirstName, user.LastName,
                                    _configuration.ApplicationName, _configuration.WebsiteBaseUrl);
                                Logger.Information(
                                    "Duplicate Account Register Post for username {userName} by requester {@requester}",
                                    userName, requester);
                            }
        ...
        public ActionResult Landing()
        {
            ...
            if (activityLogs.ToList().Count > 1) lastAccountActivity = activityLogs.Skip(1).FirstOrDefault();
